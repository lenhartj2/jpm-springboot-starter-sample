...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "demo-app-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name demo-app-service apps/demo-app-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app demo-app-service:1.0-SNAPSHOT
		call oc expose svc/demo-app-service
	) else (
