...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "demo-app-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name demo-app-service apps/demo-app-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app demo-app-service:1.0-SNAPSHOT
    oc expose svc/demo-app-service
else

	echo "Launching the application locally..."
	pattern="demo-app-service"
	files=( $pattern )
	cd ${files[0]}
